    ...
    constructor() {
        super({
            LoadFileWindow: new LoadFilesComponent(
                {
                    clickOnLoad: () => {this.closeWindow('window__load-files')}
                }
            ),
            WindowExit: new WindowExitComponent(
                {
                    clickOnExit: () => {this.closeWindow('window__exit')},
                    clickOnCancel: () => {this.closeWindow('window__exit')}
                }
            ),
            ProfileEditWindow: new EditProfileComponent(  {
                clickOnAccept: () => {this.closeWindow('window__edit')},
                clickOnCancel: () => {this.closeWindow('window__edit')}
            }),
            PasswordChangeWindow: new ChangePasswordComponent(
                {
                    clickOnAccept: () => {this.closeWindow('window__edit-password')},
            ...
            ButtonEditProfile: new Button({
                ...
                class: 'profile__edit',
                events: {
                    'click': () => { this.openWindow('window__edit') }
                }
            }),
            ButtonEditPassword: new Button({
                ...
                events: {
                    'click': () => { this.openWindow('window__edit-password') }
                }
            }),
            ButtonExit: new Button({
                ...
                class: 'profile__exit',
                events: {
                    'click': () => { this.openWindow('window__exit') }
                }
            }),
            events: {
                'click': (event: MouseEvent) => { this.findImageElement(event) }
            },
            arrow: arrow,
            avatar: avatar,
        });
